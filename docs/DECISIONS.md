...
- 后果：普通记录操作不物理删除；客户端 Rules 不允许物理删除，永久删除本人账号的数据清理由要求最近重新认证的 trusted callable 执行。Room v5 的两个业务表分别保存同步状态与远端修订号，逐次详情由对应的独立详情表保存。
...
- 数据迁移：Room v3→v4 只创建空的 `sex_records`，原自慰数据不变；这条历史边界由当前 v1→v5、v2→v5、v3→v5、v4→v5 迁移测试链继续覆盖。
