...
## ADR-002：当前手冲专用模型与未来垂直扩展

- 状态：Accepted，取代旧“通用活动模型”。
- 决策：当前产品只有 `HandBrewRecord`，不使用活动表、活动 ID 或计量类型；未来允许按独立垂直模块新增记录类型。
- 原因：当前专用模型更清楚、更易验证，同时用户希望未来扩展不必重写现有手冲逻辑。预建万能活动模型会把未知需求提前固化，独立切片能隔离数据、统计和同步语义。
- 后果：旧 `Activity`、`MeasurementType`、活动管理和筛选全部删除。未来模块必须另立 ADR，并拥有自己的实体、Repository、统计、云端路径和迁移测试；只有已经证明语义相同的基础能力可以共享。
...
- 后果：普通记录操作不物理删除；仅永久删除本人账号的数据清理流程允许删除本人路径。Room v3 保存同步状态与远端修订号。
