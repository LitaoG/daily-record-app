...
- 首版内容：账号与同步、当前记录模块、本机优先与隐私事实、版本和开源许可。账号管理复用既有弹窗，模块选择复用既有 `SelectedRecordModulePreference`。
- 原因：这些能力使用频率低于记录和统计，但分散在主页会挤压标题与同步状态；集中后可保持核心页面安静，并让用户在一个位置理解数据边界。
