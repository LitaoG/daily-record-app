...
- 状态：Accepted
- 决策：当本地记录 `remoteRevision > 0` 而云端同日文档已不存在时，提交改为以本机 pending 内容重建文档（新 revision 从 1 重新开始），并为这次物理重建生成新的 `id` 作为云端记录代际标记，不再抛出“Cloud record disappeared”数据错误。普通清除仍写墓碑，继续参与乐观并发检查；只有文档物理消失（如账号数据被清理）才触发重建。手冲与做爱模块行为一致。
