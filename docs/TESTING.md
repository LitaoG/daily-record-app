...
最小证据为手冲与做爱 Android 同步测试（含 `AccountSyncManager` 恢复、显式清除墓碑和双设备代际收敛），以及 Auth/Firestore Emulator 的真实写入、物理删除、重建和读取测试。Worker 的重试/删除闸门测试仍按上表独立执行；由于生产 Worker 通过真实 Firebase Services 组装，云端重建行为由同一 coordinator 路径和 Emulator 测试证明。
