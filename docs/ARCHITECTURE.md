...
当前公共 `main` 的运行时维护边界还包括：Compose 页面通过生命周期感知的
`collectAsStateWithLifecycle` 收集认证、同步、记录和模块数据 Flow；页面离开或进入后台时
不会继续保持不必要的 UI 重组订阅。Cloud Functions 使用 Node.js 22 的 2nd gen 运行时并
固定在 `asia-east1`，PR、`main` 和 Release tag 都在 API 34 模拟器上执行 connected 回归。
...
设置中心是 Compose 外壳中的次级页面，不进入底部主导航。它复用账号弹窗，只读取构建版本与静态隐私事实；模块选择继续由主页承载，不直接访问 DAO、Firestore，也不引入新的业务数据表或通用设置模型。
...
Room 当前版本为 6。v1→v2 只提取旧 `flight` 机器图标标识的记录，不依赖用户可见名称；旧表改名为 `legacy_*_v1` 保留作恢复证据。v2→v3 为自慰记录增加账号所有者、墓碑、同步状态和远端修订号。v3→v4 非破坏地创建空的 `sex_records`，不改写任何自慰行。v4→v5 非破坏地创建两套逐次详情表，现有聚合次数不变。v5→v6 非破坏地为两个业务表增加 `(owner_id, sync_state, updated_at)` 待同步排序索引。运行时代码不读取 legacy 表。
...
- 云端文档物理消失（如账号数据被清理）时，本机 `remoteRevision > 0` 的待同步编辑以新文档重建；重建会生成新的记录 `id` 作为云端代际标记，并从修订号 1 开始。相同 `id` 代际内按 `remoteRevision` 做乐观并发检查，发现新 `id` 时即使修订号较低也接受新代际；普通清除仍走墓碑，不会无提示地复活用户明确删除的记录（ADR-017）。本机没有待同步编辑时，完整且无格式异常的服务端快照会移除已确认的镜像行，界面回到未填写（ADR-020）。
...
- Compose 对认证、同步和页面数据 Flow 使用生命周期感知收集；页面不活跃时停止 UI 层订阅，恢复后重新收集，避免后台页面持续重组或持有不必要的观察工作。
...
- 记录页仍由计数控制承担主任务；次数大于 0 后才显示“记录时间和感受”，用户点击后才展开可编辑详情草稿。
- `HandBrewRecordDetail`/`SexRecordDetail` 与各自的 Room 表、DAO、Repository、同步映射和 Firestore 字段保持独立；UI 只使用 `RecordDetailEntry` 投影，不创建通用活动实体。
- 聚合记录的 `updated_at` 是当天计数与详情的同步版本。详情上传随各自日期文档提交，远端快照在同一 Room 事务内替换对应模块详情。
- 详情不参与当前统计；详情页清除通过保留当天聚合次数并以空详情列表原子替换本地详情，整条记录清除才删除聚合行并同步写墓碑，阻止旧设备复活。
