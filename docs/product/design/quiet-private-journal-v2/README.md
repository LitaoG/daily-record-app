...
- 周统计环图的颜色必须复用主页映射：自慰、做爱两个模块的 `1 / 2 / 3 / 4次及以上` 逐档使用对应次数块色，“0次”保持现状；“未来”使用对应模块主页“未填”日期块的 `Unset` 背景色。未来环段继续更细并可保留虚线，周几日期文字保留暗淡的未来字色；周三、周六标签相对环圈额外保留 6dp 径向间距。该定向维护由 [Issue #195](https://github.com/LitaoG/daily-record-app/issues/195) 记录。
