...
- 周统计环图的颜色必须复用主页映射：手冲、做爱两个模块的 `1 / 2 / 3 / 4次及以上` 逐档使用对应次数块色，“0次”保持现状；“未到”使用主页未来日期块的 `Disabled` 背景色。未来环段继续更细并可保留虚线，周几日期文字保留暗淡的未来字色，文字标签和 TalkBack 继续表达不同状态。该定向维护由 [Issue #180](https://github.com/LitaoG/daily-record-app/issues/180) 记录。
