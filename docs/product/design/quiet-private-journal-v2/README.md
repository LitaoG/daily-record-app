...
本目录保存 UI v2 改版当时已经确认的视觉方向。三张图分别覆盖日历、日期记录和统计，且每张图都并排展示同一页面的两个模块状态：
...
- 本视觉基线不定义时间、感受和逐次详情的视觉细节；当前运行时的逐次详情已由独立的[记录时间与感受确认稿](../record-details-v1/README.md)实现，不能再把本条理解为“当前不支持详情”。
...
- 周统计环图的颜色必须复用主页映射：自慰、做爱两个模块的 `1 / 2 / 3 / 4次及以上` 逐档使用对应次数块色，“0次”保持现状；“未来”使用主页“未来”日期块的 `DailyRecordSurfaceDisabled` 灰棕色，与主页完全一致；过去未填写（未填）复用主页对应模块“未填”日期块的极浅模块色。未来环段继续更细并可保留虚线，周几日期文字保留暗淡的未来字色；周三、周六标签相对环圈额外保留 6dp 径向间距。该定向维护由 [Issue #195](https://github.com/LitaoG/daily-record-app/issues/195) 记录。
...
当前代码的真实页面画面见[README 运行截图](../../assets/readme/README.md)。设计稿中的日期、次数和趋势数据仍是演示数据，不能替代该运行证据或测试断言。
