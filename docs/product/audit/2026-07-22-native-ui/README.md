...
> 状态：历史审计证据。本文只记录当时的应用内 UI 检查，不构成当前待办；当前事实以最新 public `main` 和 `docs/` 当前文档为准。
...
2. **清除确认——已修复。** 原实现直接使用 Material `AlertDialog`，出现淡紫色容器、默认按钮层级和项目视觉脱节；现改为共用 `DailyRecordDialog`，补充“恢复未填写”和不可撤销说明，危险操作使用独立深红主按钮。
