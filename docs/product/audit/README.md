...
当前首页运行画面不再从本目录取图。2026-08-16 的 API 34 本机模式 Debug 截图集中在[README 运行截图](../assets/readme/README.md)；它们只展示当前页面结构和空数据状态。这里的日期目录仍按原始测试时间保留，不能因“看起来旧”而改写其截图、测试数量或提交事实。
