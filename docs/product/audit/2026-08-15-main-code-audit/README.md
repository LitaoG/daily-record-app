...
- Firestore 规则要求记录带 `details` 数组、限制 `localDate` 的月日形状，并禁止 `deleted` 墓碑携带详情；规则测试覆盖了缺失字段、非法日期和墓碑数据残留。
- 详情草稿保留真实 `occurrenceIndex`，修复稀疏云端详情在下一次保存时被重排的问题；对超过 512 次的聚合记录不再按次数分配 UI 行，且计数保存不会清空未进入编辑器的详情。
...
- Firestore 读取、创建和更新按 UID 路径隔离，字段白名单、必需 `details`、合法日期形状、次数非负、稳定 ID、服务器时间戳、revision 递增以及墓碑详情为空已有规则覆盖；规则仍不能逐项遍历 `details`。
...
| 检查 | 结果 |
|---|---|
| `testDebugUnitTest` | 39 个 suite，227 tests，0 failures，0 errors |
