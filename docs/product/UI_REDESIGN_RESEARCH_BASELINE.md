...
不改变统计范围和计算；年度热力图只重新组织已有逐日数据。若当前统计模型没有暴露所需的逐日列表，可增加只读 UI 投影，但不能新建统计口径或持久化字段。
...
| 两个模块视觉差异不足 | 两个模块共用陶土色和相同强调方式 | 切换后上下文感弱 | xTracker、Kairos | 手冲深铜、做爱深梅，结构和统计口径保持相同 | 模块展示规范 | 无 | 中 |
...
- 石墨色正文，减少纯黑和大面积粗体。
- 手冲采用低饱和深铜色。
- 做爱采用低饱和深梅色，不使用艳粉、红心、身体轮廓或成人插画。
- 同步成功采用灰绿色；危险操作使用专用危险色。
- 普通内容不加阴影和常驻描边。
- 月历是首页唯一主角。
- 统计页更像个人数据摘要，不像后台管理仪表盘。

探索色板如下，尚未经过最终字号与状态矩阵验收，不能直接视为实现常量：
...
1. **设计确认，不改运行代码。**
   在同一 390×844 画布中制作日历空状态、日历混合状态和年度统计高保真稿；使用当前截图并排比较。
