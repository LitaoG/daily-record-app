...
感谢参与“手冲日历”。当前项目只记录手冲次数，优先保证数据正确、离线可用、隐私安全和交互简单；不得为尚未立项的记录类型预先建立通用活动抽象。
...
- 当前功能不引入 `Activity`、`MeasurementType` 或活动筛选。未来记录类型需要先有产品契约和 ADR，并作为独立垂直模块实现，不能污染 `HandBrewRecord`。
- 不允许用数据库清空重建代替正式迁移。
