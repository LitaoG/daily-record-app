...
- 自慰与做爱两个独立模块；切换模块时保留浏览日期和统计周期，重启后恢复上次模块。
...
当前范围只包含自慰和做爱次数，不保存伴侣、时间、地点、时长、备注、图片或医疗信息，也不包含健身、提醒、目标、社交、广告或短信验证码。两个模块拥有独立实体、表、Repository 和云端路径，不使用万能活动表。
...
- 应用不含广告、分析或崩溃上报 SDK，也不含社交功能；不会自动上传日志或错误信息。
