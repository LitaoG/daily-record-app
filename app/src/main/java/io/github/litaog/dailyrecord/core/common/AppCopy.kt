...
internal object AppCopy {
    ...
    object Auth {
        ...
        const val passwordPolicy = "密码至少 8 位，请妥善保存。"
        const val vpnNotice = "登录、注册和同步需要 VPN（梯子）。本机使用无需 VPN，但不会云同步。"
        ...
        const val offlineMergeNotice = "登录后，本机记录会合并到该账号。"
        const val cloudDataNotice = "云端只保存日期、次数和同步信息。"
        ...
        const val resetSentSubtitle = "邮件已发送；为保护隐私，不显示邮箱是否已注册。"
        const val resetSubtitle = "输入注册邮箱，我们会发送重置邮件。"
        const val resetSuccess = "请查收重置邮件（包括垃圾邮件），按邮件提示修改密码。"
    ...
    object Account {
        ...
        const val syncDescription = "记录先保存在本机；联网后自动同步，换手机登录即可恢复。"
        ...
        const val networkFailureGuidance = "检查网络或 VPN（梯子）后重试。"
        ...
        const val dataFailureGuidance = "原始记录仍在本机，请稍后重试。"
        const val unknownFailureTitle = "暂时无法完成同步"
        const val unknownFailureGuidance = "原因暂时无法确定，记录仍在本机，请稍后重试。"
        const val syncDialogMessage = "检查网络或 VPN（梯子）后重试。"
    ...
    object Calendar {
        ...
        const val legendDescription = "点击日期填写%s次数。状态：未填写、未来不可填写、0次、已记录"
    ...
    object Record {
        ...
        const val clearMessage = "清除后无法在应用内恢复，也不会计入统计。"
    ...
    object Statistics {
        ...
        const val recordedDaysLabel = "记录天数"
        ...
        fun monthSummaryAccessibility(totalCount: Long, recordedDays: Int, average: Double): String =
            "本月总次数${countText(totalCount)}；记录天数${daysText(recordedDays)}；日均次数${average(average)}"
