...
internal object AppCopy {
    ...
    object Auth {
        ...
        const val subtitle = "登录后，本机记录会与账号合并；换手机登录即可恢复。"
        const val passwordPolicy = "密码至少 8 位，请妥善保存。"
        const val vpnNotice = "登录、注册和云同步需要 VPN（梯子）。离线使用不需要。"
        ...
        const val offlineMergeNotice = "登录后，本机记录会与账号合并。"
        const val cloudDataNotice = "云端只保存日期、次数和同步状态。"
