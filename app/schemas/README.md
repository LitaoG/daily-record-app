...
- `6.json` 是当前双模块数据库结构（Room v6），业务表为相互独立的 `hand_brew_records`、`hand_brew_record_details`、`sex_records` 和 `sex_record_details`；v6 相对 v5 只新增两个业务表的待同步排序索引，不改变行数据。
- `5.json` 是增加待同步排序索引前的双模块数据库结构。
